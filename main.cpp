...
int main (int argc, char* argv[])
{
...

    {
        ...
        Vector<DistributionMapping> dmap;
        {
            ...
            geom_coarse.define(domain, rb, CoordSys::cartesian, is_periodic);
            geom.push_back(geom_coarse);

            BoxArray grid_coarse(domain); // define the BoxArray to be a single grid
        ...
        EB2::Build(geom[0], required_coarsening_level, max_coarsening_level);
        ...
        EBFArrayBoxFactory factory(eb_level, geom[0], grids[0], dmap[0], ng_ebs, ebs);

        // charge density and electric potential
        MultiFab q  (grids[0], dmap[0], 1, 0, MFInfo(), factory);
        MultiFab phi(grids[0], dmap[0], 1, 0, MFInfo(), factory);
        ...
        // set ACoef to zero
        MultiFab acoef(grids[0], dmap[0], 1, 0, MFInfo(), factory);
        acoef.setVal(0.);
        ...
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            bcoef[idim].define(amrex::convert(grids[0],IntVect::TheDimensionVector(idim)), dmap[0], 1, 0, MFInfo(), factory);
        ...
        MultiFab beta(grids[0], dmap[0], 1, 0, MFInfo(), factory);
        beta.setVal(1.);
        ...
        MultiFab plotfile_mf(grids[0], dmap[0], 2, 0, MFInfo(), factory);
        MultiFab::Copy(plotfile_mf,  q,0,0,1,0);
        MultiFab::Copy(plotfile_mf,phi,0,1,1,0);

        EB_WriteSingleLevelPlotfile("plt", plotfile_mf, {"q", "phi"}, geom[0], 0.0, 0);
    }
